...
class CombatMedalTests(unittest.TestCase):
    ...
    def test_preview_covers_six_events_and_three_rope_tiers(self):
        self.lua.execute('''
            local seen = {}
            for i = 1, 13 do
            ...
            for _, id in ipairs({'first_strike','grave','low_hp','reload','run'}) do
    ...
    def test_event_localizations(self):
        import json
        ids = ['first_strike', 'grave', 'low_hp', 'reload', 'run', 'rope', 'rope_3', 'rope_5']
        ...
        for language in ('english', 'french'):
            ...
            self.assertNotIn('ky_hud_event_medal_headshot', data)
    ...
    def test_host_predeath_capture_and_client_hooks(self):
        self.lua.execute('''
            ...
            for _, id in ipairs({'first_strike','grave','low_hp','reload','run','rope'}) do
            ...
            assert(count_event('first_strike') == 2 and count_event('reload') == 2 and count_event('run') == 2)
            local before = #cards
    ...
    def test_hidden_hud_consumes_first_strike_and_rope_tiers(self):
        self.lua.execute('''
            Hooks.callbacks.KH_EventAssaultStart({}, 1)
            ...
            kill({rope=true})
            assert(#cards == 0, 'hidden award replayed')
            kill({rope=true})
            assert(count_event('first_strike') == 0 and count_event('rope') == 1)
            assert(cards[1].tier_index == 3)
        ''')
