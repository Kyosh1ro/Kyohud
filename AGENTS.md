...
- Le code source de Void UI disponible dans `G:\PD2-Modding-Docs\Void UI` peut servir de référence pour le cycle de vie des panneaux, le chargement conditionnel par `RequiredScript`, l'enregistrement de textures et la coexistence entre composants HUD. Il ne constitue ni une dépendance ni un modèle à recopier sans vérifier ses choix de hooks.
- Le code source d'Extra Heist Info disponible dans `G:\PD2-Modding-Docs\Extra Heist Info` peut servir de référence pour les menus déclaratifs JSON, la conversion des entrées en appels `MenuHelper` et les dépendances entre options. Son parseur étend largement SuperBLT et ne doit pas être copié intégralement dans ce mod.
